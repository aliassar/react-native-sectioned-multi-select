...
class SectionedMultiSelect extends PureComponent {
    ...
    find = (id, items) => {
        ...
        for (; i < items.length; i += 1) {
            if (items[i][uniqueKey] === id) {
                return {found:items[i], parent: items[i][uniqueKey]}
            } else if (Array.isArray(items[i][subKey])) {
                foundObject = this.find(id, items[i][subKey])
                found=foundObject && foundObject.found
                if (found) {
                    return {found, parent: items[i][uniqueKey]}
                }
            }
        }
    }
    ...
    _getSelectLabel = () => {
        ...
        if (!selectedItems || selectedItems.length === 0) {
            ...
            const foundItemObject = this._findItem(item)
            const foundItem=foundItemObject && foundItemObject.found
    ...
    _selectAllItems = () => {
        ...
        items.forEach((item) => {
            ...
            parents = [...parents,item[uniqueKey]]
        })


        onSelectedItemsChange(newItems)
        onSelectedItemObjectsChange && onSelectedItemObjectsChange(items,parents)
    }
    ...
    _broadcastItemObjects = (newItems) => {
        ...
        newItems.forEach((singleSelectedItem) => {
            ...
            fullItems.push(item.found)
            fullParents.push(item.parent)
        })
        onSelectedItemObjectsChange(fullItems,fullParents)
    }
    ...
    _displaySelectedItems = () => {
        ...
        return selectedItems.map((singleSelectedItem) => {
            ...
            const itemObject = this._findItem(singleSelectedItem)
            const item=itemObject.found
    ...
    ModalContentWrapper = ({children}) => {
        ...
        return <Component
           style={[{flex: 1, backgroundColor: 'rgba(0,0,0,0.5)'}, styles.modalWrapper]}>{children}</Component>
    }
                                      ...
                                      >
                                          <Text
                                             style={[{
                                                 fontSize: 18,
                                                 color: '#ffffff'
                                             }, confirmFont, styles.confirmText]}
                                          >
                                              {confirmText}
                                          </Text>
                                      </View>
